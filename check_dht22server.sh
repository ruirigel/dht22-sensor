...
echo "`hostname -I | cut -f1 -d ' '` - - [`date "+%d/%b/%Y %H:%M:%S"`] Checking server." >> dht22server_log.txt

code=$(curl --write-out %{http_code} --silent --output /dev/null localhost:8000/plot.png)
